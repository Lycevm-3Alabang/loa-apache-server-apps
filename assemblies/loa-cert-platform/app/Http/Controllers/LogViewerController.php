...
class LogViewerController extends Controller
{
    ...
    {
        $secret = config('cert-platform.log_viewer_secret', '');

        if (!$secret || request()->input('password') !== $secret) {
            return redirect()->route('logs.login')->withErrors(['password' => 'Invalid password.']);
        }

        session(['log_viewer_authed' => true]);
    ...
    {
        session()->forget('log_viewer_authed');
