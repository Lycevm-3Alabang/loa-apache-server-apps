...
class ResultsTest extends TestCase
{
    ...
    private const PATTERNS = [
        '/api/v1/admin/evaluation-results',
        '/api/v1/admin/evaluation-results/departments/{departmentId}',
        '/api/v1/admin/evaluation-results/faculty/{facultyId}',
        '/api/v1/admin/evaluation-results/groups/{facultySubjectId}',
        ...
        '/api/v1/dean/evaluation-results/department',
        '/api/v1/dean/evaluation-results/departments/{departmentId}',
        '/api/v1/dean/evaluation-results/departments/{departmentId}/faculty/{facultyId}',
        '/api/v1/dean/evaluation-results/departments/{departmentId}/groups/{facultySubjectId}',
        '/api/v1/dean/evaluation-results/details',
        '/api/v1/faculty/evaluation-results',
        '/api/v1/faculty/evaluation-results/subjects',
        '/api/v1/faculty/evaluation-results/subjects/{facultySubjectId}',
    ];
    ...
    {
        $w = $this->world();
        $h = $this->auth('a@lyceumalabang.edu.ph', ['ADMIN']);

        // Lazy compute fires through the read path (no rows stored yet).
        $response = $this->getJson('/api/v1/admin/evaluation-results?evaluationPeriodId=' . $w['period']->id, $h);
        ...
        $this->getJson('/api/v1/admin/evaluation-results', $h)->assertStatus(400);
    }
    ...
    public function test_admin_drill_downs(): void
    {
        ...
        $this->getJson('/api/v1/admin/evaluation-results/departments/999999' . $q, $h)->assertNotFound();
        $this->getJson('/api/v1/admin/evaluation-results/departments/' . $w['dept']->id . $q, $h)
            ->assertOk()->assertJsonStructure(['department', 'subjects']);
        $this->getJson('/api/v1/admin/evaluation-results/faculty/999999' . $q, $h)->assertNotFound();
        $this->getJson('/api/v1/admin/evaluation-results/faculty/' . $w['faculty']->id . $q, $h)
            ->assertOk()->assertJsonStructure(['faculty', 'subjects']);
        $this->getJson('/api/v1/admin/evaluation-results/groups/' . $w['mapping']->id . $q, $h)
    ...
    {
        $w = $this->world();
        $h = $this->auth('f@lyceumalabang.edu.ph', ['FACULTY']);
        $q = '?evaluationPeriodId=' . $w['period']->id;

        $this->getJson('/api/v1/faculty/evaluation-results' . $q, $h)->assertForbidden();
        $this->getJson('/api/v1/faculty/evaluation-results/subjects' . $q, $h)->assertForbidden();

        $admin = $this->auth('a@lyceumalabang.edu.ph', ['ADMIN']);
        $this->postJson('/api/v1/admin/evaluation-results/visibility', [
            'evaluationPeriodId' => $w['period']->id,
            'facultyIds' => [$w['faculty']->id],
            'visible' => true,
        ], $admin)->assertOk()->assertJson(['success' => true]);

        $this->getJson('/api/v1/faculty/evaluation-results' . $q, $h)
            ->assertOk()->assertJsonStructure(['results', 'facultyNames'])
            ->assertJsonPath('results.0.totalRespondents', 1);
        $this->getJson('/api/v1/faculty/evaluation-results/subjects' . $q, $h)
            ->assertOk()->assertJsonCount(1, 'subjects');
        $this->getJson('/api/v1/faculty/evaluation-results/subjects/' . $w['mapping']->id . $q, $h)
            ->assertOk()->assertJsonStructure(['subject']);
        $this->getJson('/api/v1/faculty/evaluation-results/subjects/999999' . $q, $h)->assertNotFound();
    }

    public function test_dean_scoping_and_details(): void
    {
        $w = $this->world();
        $dean = $this->auth('dean@lyceumalabang.edu.ph', ['DEAN']);
        $q = '?evaluationPeriodId=' . $w['period']->id;

        $this->getJson('/api/v1/dean/evaluation-results' . $q, $dean)
            ->assertOk()->assertJsonCount(1, 'departments');
        $this->getJson('/api/v1/dean/evaluation-results/department', $dean)
            ->assertOk()->assertJson(['departmentId' => $w['dept']->id]);
        $this->getJson('/api/v1/dean/evaluation-results/departments/999999', $dean)->assertForbidden();

        $details = $this->getJson(
            '/api/v1/dean/evaluation-results/details?evaluationPeriodId=' . $w['period']->id . '&facultyId=' . $w['faculty']->id,
            $dean
    ...
    public function test_mutations_and_disabled_set(): void
    {
        ...
        $this->postJson('/api/v1/admin/evaluation-results/invalidate', [
            'evaluationPeriodId' => $w['period']->id,
        ], $h)->assertStatus(400);
        $this->postJson('/api/v1/admin/evaluation-results/invalidate', [
        ...
        $this->getJson('/api/v1/admin/evaluations/disabled', $h)
            ->assertOk()->assertJsonCount(1, 'evaluations');

        $this->postJson('/api/v1/admin/evaluations/disabled/restore', ['ids' => []], $h)->assertStatus(400);
        $this->postJson('/api/v1/admin/evaluations/disabled/restore', ['ids' => [$w['eval']->id]], $h)
            ->assertOk();
        $this->assertFalse($w['eval']->fresh()->is_disabled);

        $this->deleteJson('/api/v1/admin/evaluations/disabled', [], $h)->assertStatus(400);
        $this->postJson('/api/v1/admin/evaluations/' . $w['eval']->id . '/invalidate', [
            'evaluationPeriodId' => $w['period']->id, 'reason' => 'test',
        ], $h)->assertOk()->assertJson(['success' => true]);
        $this->assertEquals('INVALID', $w['eval']->fresh()->status);

        $this->getJson('/api/v1/admin/evaluations/' . $w['eval']->id . '/details', $h)
            ->assertOk()
            ->assertJsonStructure(['evaluationId', 'categories', 'comment', 'sentimentScore', 'isDisabled']);
        $this->getJson('/api/v1/admin/evaluations/999999/details', $h)->assertNotFound();

        $this->deleteJson('/api/v1/admin/evaluations/disabled', ['all' => true], $h)
