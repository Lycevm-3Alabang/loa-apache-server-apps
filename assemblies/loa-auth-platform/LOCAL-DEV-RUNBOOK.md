...
|----------------|--------------------------------------|----------------------------------|
| `auth-app`     | PHP 8.3 application container         | — (exposed via nginx)            |
...
docker compose run --rm auth-app composer install --no-interaction --no-progress

# Run migrations
docker compose run --rm auth-app php artisan migrate --force

# Run database seeder
docker compose run --rm auth-app php artisan db:seed --force

# Generate Swagger documentation
docker compose run --rm auth-app php artisan l5-swagger:generate

# Generate application key (if APP_KEY not set in .env)
docker compose run --rm auth-app php artisan key:generate --force
```
...
| Laravel Artisan   | Pre-installed in container           | Use `docker compose run --rm auth-app php artisan <command>`               |
| PHPUnit           | Vendored in `vendor/bin/phpunit`     | Run tests with `docker compose run --rm auth-app php artisan test`          |
...
3. The `auth-app` container receives `SEQ_URL=http://seq:5341` from the compose environment.
...
```bash
docker compose run --rm auth-app php artisan migrate:reset --force
```
...
```bash
docker compose run --rm auth-app php artisan test
```

### Run a Specific Test File

```bash
docker compose run --rm auth-app php artisan test tests/Feature/Auth/LoginTest.php
```

### Run a Specific Test Method

```bash
docker compose run --rm auth-app php artisan test --filter testLoginSuccess
```
...
```bash
docker compose run --rm auth-app bash
```
...
- For email testing, Mailpit is available at `http://localhost:8026`.
...
3. Ensure migrations have run: `docker compose run --rm auth-app php artisan migrate:status`

### Seeder Fails

1. Confirm migrations completed successfully
2. Check `.env` for required values: `APP_KEY`, `ADMIN_EMAIL`, `ADMIN_PASSWORD`
3. Inspect seeder output: `docker compose run --rm auth-app php artisan db:seed --force -v`

### Seq Not Receiving Logs

1. Confirm `seq` service is running: `docker compose ps seq`
2. Check `SEQ_URL` is set in `auth-app` environment: `docker compose exec auth-app env | grep SEQ`
