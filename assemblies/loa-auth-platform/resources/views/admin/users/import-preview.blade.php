...
<div class="page-header">
    <div>
        ...
        <p>Review imported rows before processing.</p>
    </div>
    <a class="button button-ghost" href="{{ route('admin.users.import') }}" style="border-color:var(--border);color:var(--text-secondary);">Start Over</a>
</div>
...
{{-- Preview Table --}}
<form id="import-process-form" method="post" action="{{ route('admin.users.import.process') }}">
    @csrf
...
<div class="detail-card" style="margin-top:1.5rem;" id="confirm-and-process">
    ...
    <label style="display:flex;align-items:center;gap:0.5rem;cursor:pointer;">
        ...
        I understand this will create/update users and assign them to tenants and groups.
    </label>
...
<script>
    ...
    document.getElementById('process-btn').addEventListener('click', function() {
        ...
        fetch('{{ route('admin.users.import.process') }}', {
            ...
            body: new URLSearchParams(new FormData(document.getElementById('import-process-form')))
        })
        ...
        .then(data => {
            ...
            if (data.status === 'applied') {
                // Refresh the results component via AJAX (spec §9.5)
                refreshResults(data);
    ...
    function refreshResults(data) {
        ...
        if (data.failed_rows && data.failed_rows.length > 0) {
            html += '<div style="margin-bottom:1rem;"><a href="{{ route('admin.users.import.failed') }}" class="button button-ghost">Download Failed Rows</a></div>';
        }

        html += '<a class="button button-ghost" href="{{ route('admin.users.import') }}" style="border-color:var(--border);color:var(--text-secondary);margin-top:1rem;">Start Over</a>';
        html += '</div>';
