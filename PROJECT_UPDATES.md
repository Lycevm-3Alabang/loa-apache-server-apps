...
- **Key specs:** `api-endpoints.md` (**Final v1.3** — 50 domain endpoints: 48 JWT-gated + 2 public), `legacy-e-cert-integration.md` (**Final v2.0**; authoritative retrofit spec, synced to `D:\loa\e-cert\legacy-e-cert-integration.md` per D7), `web-ui.md`, `README.md`.
...
- **Phase A COMPLETE (2026-08-06):** user confirmed decision #17 (Cert-proxied refresh/logout) and dashboard stats at `read` (with ownership note). Promoted `api-endpoints.md` → **Final v1.3** (§9.2 SSO URL → `/sso/login`, §9.9 `/access` optional, §5.7 dashboard ownership note, decision #17 confirmed, example numbers → `CERT-0001`) and `legacy-e-cert-integration.md` → **Final v2.0** (§7.2 dashboard ownership note, §12 Phase A complete). D7 copy re-synced.

### In Progress
- **Phase B (next):** Auth readiness — redirect allowlist, cert catalog import (Appendix A), seed `cert-admin`/`cert-staff`/`cert-user` groups + grants

### Next Action
- [x] **Phase A — COMPLETE 2026-08-06:** `api-endpoints.md` v1.3 and `legacy-e-cert-integration.md` v2.0 → **Final**
- [ ] **Phase B:** Auth readiness — redirect allowlist incl. `https://e-cert.vercel.app`, cert catalog import (Appendix A), seed `cert-admin`/`cert-staff`/`cert-user` groups + grants
...
| 2026-08-06 (6) | **`cert-readiness.md` → Final v0.2:** added **§8 Local Development** (Docker Compose) — local admin UI at `localhost:8080`, local origin/CORS/redirect table (incl. local e-cert dev-server origins), optional ad-hoc tinker fast path for tenant+groups (not in any seeder), local verification; §3 cross-ref; References + Doc Control renumbered to §9–§11 | Deploy Auth → provision per runbook → Phase C (Cert scaffold) |
