...
| 2026-08-24 (6) | **/login admission tests added** (`SsoAuthTest`, 4 new): admin → web session + refresh-token revocation; tenant member + valid `?redirect=` → splash delivery without web session; no redirect / unknown origin → rejected with tokens revoked. Full suite 228/563 green | Auth/cert deploy |
| 2026-08-24 (7) | **cPanel SMTP wired locally for BOTH apps**: removed hardcoded cert-app `MAIL_*` from root compose (mail now driven by each app's bind-mounted `.env`, matching auth); both local `.env` → `mail.lyceumalabang.edu.ph:587` as `noreply@` (untracked). **Bug fix:** auth `config/mail.php` mapped legacy `MAIL_ENCRYPTION` into the mailer `scheme` key — `tls` is invalid there (auth send FAILED; cert worked via framework default). Config now reads only `MAIL_SCHEME`; STARTTLS auto on 587. Both apps verified SENT end-to-end. `.env.example`s default to Mailpit; DEPLOY.md + web-ui.md §7 updated (no `MAIL_ENCRYPTION`) | Auth/cert deploy |
| 2026-08-24 (8) | **Refresh-token flow verified live + 2 bugs fixed (cert):** probing `/api/v1/auth/refresh` exposed `JsonResponse->cookies` crash in ALL branches of `AuthRefreshController` (500 on logout/refresh paths; same bug in `AuthLogoutController`) — rewritten with `Cookie::queue()`; local config gap: cert `.env` had no `AUTH_BASE_URL` → defaulted to undeployed prod host → now `http://auth-nginx` (+ example note). E2E verified: auth-issued refresh via cookie → proxy → rotation → new access token. Suite 168/557 green. e-cert SPA: stale-token clear on refresh-fail (`6af8b0d`); forgot/reset back-link now "Back to referrer" with HTTP-referer fallback | Auth/cert deploy |
| 2026-08-24 (9) | **419 "Page Expired" eliminated on auth forms** (user-reported nuisance): `bootstrap/app.php` render handler converts expired-CSRF POSTs into fresh-form redirects with "Your session has expired" flash — per-route re-render map (`/login`, `/sso/*`, `/forgot-password`, `/reset-password` carrying `token`+`email`; admin forms → back()). Framework quirk: TokenMismatchException is converted to `HttpException(419)` BEFORE render callbacks → must type-hint the latter. Spec'd web-ui.md §4.0; new `CsrfExpiryTest` (4 tests, CSRF enabled). Suite 234/583 green | Auth/cert deploy |
| 2026-08-24 (10) | **Parametrized dist builds**: rewrote auth `generate-dist.ps1` + new cert twin — both take mandatory `-Path` (destination created on demand), emit `<app>-dist\` folder + `.zip`, self-locating via `$PSScriptRoot`. Excludes git/vendor/node_modules/docker/compose/cert-app leftover, phpunit caches, `tmp/`, stray `loa_auth` sqlite, self; strips `.env*`/tests/docs (README only) and empties `storage/framework/{cache,sessions,views,logs}`. Verified clean output both apps | Auth/cert deploy |
| 2026-08-26 | **Admin dashboard home implemented** per `admin-dashboard-home.md` v1.0 Final: controller data assembly (stat cards, attention queue, activity feed), admin-zone Blade partial, `?status=pending` fix, dashboard `@include`. Zero-JS, H4 fail-degrade | Run tests + lint; Phase D e-cert auth swap |
| 2026-08-26 (2) | **§12 Group-Permission-Management implemented** (auth): spec promoted to Final v3.0; I1/M8 guards on AuthorizationService; 5 new routes (tenant group members search/add/remove/confirm + platform-permissions toggle), 2 old user-detail routes removed; 6 new controller methods, 2 modified with M6 guards; user detail rewrote to read-only, group-members rewrote with two-tier search, new member-remove-confirm interstitial, platform group show scoped; `auth:repair-i1-violations` artisan command; 25 new tests; fixed search table name + M6 guards + audit test routes | Run tests + lint; Phase D e-cert auth swap |
